#!/bin/bash
# MobileSAM fine-tuned on ARCADE — ablation=1 (GT loss only, no KD, no clDice)
# Same 900/100/200 clean split as v3. This is the direct fine-tuning baseline:
# answers "how much does distillation add over just training on labels directly?"
set -euo pipefail
LOG=/tmp/mobilesam_finetune.log
exec > >(tee -a $LOG) 2>&1
trap 'gsutil cp $LOG gs://coronary-angio-v2/results/distillation/mobilesam_finetune_run.log 2>/dev/null || true; shutdown -h now' EXIT

echo "=== MobileSAM fine-tune baseline (ablation=1, 50 epochs, clean split) $(date) ==="
...
# Teacher checkpoint needed only to transplant decoder weights into student
python3 -c "
...
gsutil cp /home/jupyter/mobilesam_abl1_finetune.pt \
  gs://coronary-angio-v2/checkpoints/mobilesam_abl1_finetune.pt
echo "Done $(date)"
