#!/bin/bash
# MobileSAM v3: clean 900/100/200 split — no data leakage
# - 900 train + 100 val (early stopping) from 1000 ARCADE train images, seed=42
# - 200 ARCADE val images downloaded as arcade_test (held-out, never touched in train())
set -euo pipefail
LOG=/tmp/mobilesam_v3.log
exec > >(tee -a $LOG) 2>&1
trap 'gsutil cp $LOG gs://coronary-angio-v2/results/distillation/mobilesam_v3_run.log 2>/dev/null || true; shutdown -h now' EXIT

echo "=== MobileSAM v3 (50 epochs, early stopping, clean split) $(date) ==="
...
# 1000 training images — train() will split 900/100 internally
...
# 200 ARCADE val images = held-out test set (downloaded to arcade_test, never used for decisions)
...
gsutil cp /home/jupyter/mobilesam_abl4_v3.pt gs://coronary-angio-v2/checkpoints/mobilesam_abl4_v3.pt
echo "Done $(date)"
